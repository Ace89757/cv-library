...
class AlchemyCocoDet2dMetric(CocoMetric):
    ...
    def _obtain_cat_infos(self) -> dict:
        ...
        for cat in self.dataset_meta['classes']:
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if self.cat_infos is None:
            self.cat_infos = self._obtain_cat_infos()
        ...
        if self.classwise:
            ...
            for idx, cat_id in enumerate(self.cat_ids):
                ...
                cat_name = nm["name"]
                t = [f'{cat_name}', f'{self.cat_infos[cat_name]["imgs"]}', f'{self.cat_infos[cat_name]["anns"]}']
            ...
            headers = [
                'category', 'images', 'bboxes', 'AP', 'AR', 'F1', 'AP@.5', 'AP@.75', 'AP@small', 'AP@medium', 'AP@large'
            ]
        ...
        heading_line(log='Average Metric', logger=logger)
        for metric_item in metric_items:
            val = float(f'{round(coco_eval.stats[coco_metric_names[metric_item]], 3)}')
            eval_results[metric_item] = val

            # best
            if metric_item not in self.best:
                self.best[metric_item] = (self.current_epoch, val)
            else:
                if val > self.best[metric_item][1]:
                    self.best[metric_item] = (self.current_epoch, val)

            # print
            current_str = '{: <20}'.format(f'{metric_item}: {val}')
            best_str = f'(best: {self.best[metric_item][1]} [{self.best[metric_item][0]}])'
        ...
        if 'mF1' not in self.best:
            self.best['mF1'] = (self.current_epoch, f1)
        else:
            if f1 > self.best['mF1'][1]:
                self.best['mF1'] = (self.current_epoch, f1)

        current_str = '{: <20}'.format(f'mF1: {f1}')
        best_str = f'(best: {self.best["mF1"][1]} [{self.best["mF1"][0]}])'
        logger.info(f'{current_str}{best_str}')
